...
# Send token request.
def reqToken(loginLink, altLoginLink, authHeader):
    count = 0
    while True:
        count += 1
...
config, configPath, boolValues = [ConfigParser(), os.path.join(os.path.split(os.path.abspath(__file__))[0], "config.ini"), ["true", "false"]]
if not os.path.exists(configPath):
    ...
    if bStartSetup == "1":
        ...
        iList = []
        dumpOptionsJson = {"Dump_Campaign": "dump the campaign profile", "Dump_Common_Public": "dump the common_public profile", "Dump_Contentpages": "recreate and dump the profile0"}
        for option in dumpOptionsJson: iList.append(validInput(f"Do you want the program to {dumpOptionsJson[option]}?\nValid vaules: {', '.join(boolValues)}.", boolValues))
        iDump_Campaign, iDump_Common_Public, iDump_Profile0 = iList
    else: iAuthorization_Type, iDump_Campaign, iDump_Common_Public, iDump_Profile0 = ["token", "true", "true", "true"]
    with open(configPath, "w") as configFile: configFile.write(f"[Anyones_Fortnite_STW_Profile_Dumper_Config]\n\n# Which authentication method do you want the program to use?\n# Token auth metod generates a refresh token to log in. After 23 days of not using this program this token will expire and you will have to regenerate the auth file.\n# Device auth method generates authorization credentials that don't have an expiration date, but can after some time cause epic to ask you to change your password.\n# Valid vaules: token, device.\nAuthorization_Type = {iAuthorization_Type}\n\n# Do you want the program to dump the campaign profile?\n# Valid vaules: true, false.\nDump_Campaign = {iDump_Campaign}\n\n# Do you want the program to dump the common_public profile?\n# Valid vaules: true, false.\nDump_Common_Public = {iDump_Common_Public}\n\n# Do you want the program to recreate and dump the profile0 profile?\n# Valid vaules: true, false.\nDump_Profile0 = {iDump_Profile0}\n\n# Do not change anything below.\n[Config_Version]\nVersion = AFSTWPDC_{configVersion}")
    print("The config.ini file was generated successfully.\n")
try:
    config.read(configPath)
    configVer, authType, bDumpCampaign, bDumpCommonPublic, bDumpProfile0 = [config['Config_Version']['Version'], config['Anyones_Fortnite_STW_Profile_Dumper_Config']['Authorization_Type'].lower(), config['Anyones_Fortnite_STW_Profile_Dumper_Config']['Dump_Campaign'].lower(), config['Anyones_Fortnite_STW_Profile_Dumper_Config']['Dump_Common_Public'].lower(), config['Anyones_Fortnite_STW_Profile_Dumper_Config']['Dump_Profile0'].lower()]
...
checkValuesJson, profilesList = [{"Authorization_Type": {"value": authType, "validValues": ["token", "device"]}, "Dump_Campaign": {"value": bDumpCampaign, "validValues": boolValues}, "Dump_Common_Public": {"value": bDumpCommonPublic, "validValues": boolValues}, "Dump_Profile0": {"value": bDumpProfile0, "validValues": boolValues}}, {"Dump_Campaign": "campaign", "Dump_Common_Public": "common_public", "Dump_Profile0": "profile0"}]
...
if not os.path.exists(authPath):
    ...
    if authType == "token":
        reqToken = reqToken(loginLink.format("34a02cf8f4414e29b15921876da36f9a"), links.loginLink1.format("34a02cf8f4414e29b15921876da36f9a"), "MzRhMDJjZjhmNDQxNGUyOWIxNTkyMTg3NmRhMzZmOWE6ZGFhZmJjY2M3Mzc3NDUwMzlkZmZlNTNkOTRmYzc2Y2Y=")
        ...
        reqToken = reqToken(loginLink.format("3446cd72694c4a4485d81b77adbb2141"), links.loginLink1.format("3446cd72694c4a4485d81b77adbb2141"), "MzQ0NmNkNzI2OTRjNGE0NDg1ZDgxYjc3YWRiYjIxNDE6OTIwOWQ0YTVlMjVhNDU3ZmI5YjA3NDg5ZDMxM2I0MWE=")
...
if len(profilesList) >= 1:
    ...
    profilePath = os.path.join(os.path.split(os.path.abspath(__file__))[0], "Dumped files", f"{displayName}'s STW Profiles", currentDate)
    ...
    for profile in profilesList:
        profile, bProfileDumped = [profilesList[profile], False]
        profileCount += 1
        profileFilePath = os.path.join(profilePath, f"{profile}.json")
        if profile == "profile0":
            campaignPath, commonPublicPath = [os.path.join(profilePath, "campaign.json"), os.path.join(profilePath, "common_public.json")]
            if (os.path.exists(campaignPath) and os.path.exists(commonPublicPath)):
                with open(campaignPath, "r") as getCampaign, open(commonPublicPath, "r") as getCommonPublic: campaign, common_public = [json.loads(getCampaign.read()), json.loads(getCommonPublic.read())]
